...
from adr import adr_vp_teacher_forcing, adr_vp_feedback
...
def main():
    ...
    evaluate_adr_vp(frames,
                    ...
                    steps=steps,
                    ec_load_name='Ec_o_overfit.h5',
                    a_load_name='A_o_overfit.h5',
                    eo_load_name='Eo_overfit.h5',
                    da_load_name='Da_o_overfit.h5',
                    do_load_name='D_o_overfit.h5',
                    la_load_name='La_o_overfit.h5',
                    l_load_name='L_overfit.h5',
                    config=config,
                    evaluate=True,
                    predict=True)
...
def evaluate_adr_vp(frames, actions, states, context_frames, ckpt_dir=None, hc_dim=128, ha_dim=16, ho_dim=32, za_dim=10,
                    ...
                    evaluate=False, predict=True, steps=1, feedback_predictions=True):
    ...
    Do = load_decoder(h_dim=hc_dim + ha_dim + ho_dim, model_name='Do', ckpt_dir=ckpt_dir, output_channels=6,
                      filename=do_load_name, output_activation='sigmoid', trainable=False, load_model_state=False)

    Eo = load_encoder(batch_shape=[bs, 1, w, h, c * 2], h_dim=ho_dim, model_name='Eo', ckpt_dir=ckpt_dir,
                      filename=eo_load_name, trainable=False, load_model_state=False)

    L = load_lstm(batch_shape=[bs, seq_len, hc_dim + ha_dim * 2 + ho_dim], output_dim=ho_dim, n_layers=lstm_layers,
    ...
    if gaussian_a:
        ...
        La = load_lstm(batch_shape=[bs, seq_len, hc_dim + ha_dim], output_dim=za_dim, lstm_units=lstm_a_units,
                       n_layers=lstm_a_layers, ckpt_dir=ckpt_dir, filename=la_load_name, lstm_type='gaussian',
                       name='La', trainable=False, load_model_state=False)
    else:
    ...
    if feedback_predictions:
        model = adr_vp_feedback(frames, actions, states, context_frames, Ec=Ec, Eo=Eo, A=A, Do=Do, Da=Da, L=L, La=La,
    ...
    if predict:
        _, _, x, _, _, x_a, imgs = model.predict(x=None, steps=steps)
